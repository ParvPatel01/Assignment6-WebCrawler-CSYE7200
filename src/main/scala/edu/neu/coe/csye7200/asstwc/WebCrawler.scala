...
object WebCrawler extends App {
  ...
  def fetchAndParseLinks(url: URL)(implicit ec: ExecutionContext): Future[Seq[URL]] =
    ...
    for {
      ...
      linksTry <- asFuture(getLinks(content, url))
    } yield linksTry
  // END SOLUTION
...
// TO BE IMPLEMENTED
  {
    val anchorTags = node \\ "a"
    anchorTags.flatMap { anchor =>
      anchor.attribute("href").map(_.text).filter(isValidURLString).map { href =>
        createRelURL(Some(url), href)
      }
    }
  }
// END SOLUTION
